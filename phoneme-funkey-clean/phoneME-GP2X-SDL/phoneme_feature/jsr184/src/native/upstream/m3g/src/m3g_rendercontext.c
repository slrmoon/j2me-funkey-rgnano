...
static M3Gbool m3gValidateDepthBuffer(RenderContext *ctx);
...
static M3Gbool m3gCanDirectRender(const RenderContext *ctx)
{
    return ctx->target.type == SURFACE_MEMORY && ctx->target.pixels != NULL;
}
                                ...
                                M3Gint scope)
{
    ...
    M3G_UNREF(ib);
    M3G_UNREF(app);
    M3G_UNREF(modelTransform);
    M3G_UNREF(alphaFactor);
    M3G_UNREF(scope);
    return M3G_FALSE;
}
...
static void m3gMakeGLCurrent(RenderContext *ctx)
{
    if (ctx != NULL && ctx->target.type != SURFACE_NONE) {
        nglSetRenderTarget(ctx->target.pixels,
                           ctx->target.width,
                           ctx->target.height,
                           ctx->target.stride);
    }
...
{
    M3G_UNREF(ctx);
    M3G_UNREF(surfaceTypes);
    M3G_UNREF(targetHandle);
}
#endif
...
{
#if defined(M3G_ENABLE_GLES_RESOURCE_HANDLING) && !defined(M3G_NGL_CONTEXT_API)
...
#if defined(M3G_NGL_CONTEXT_API)
M3G_API void m3gBindBitmapTarget(M3GRenderContext hCtx,
