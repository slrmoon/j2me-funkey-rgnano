                               ...
                               int line)
{
	...
	printf("[native] Error %d at %s, line %d\n", (int) errorCode, filename, line);
...
static void m3gDebugRaiseError(Interface *m3g,
                               ...
                               int line)
#else
static void m3gRaiseError5(Interface *m3g, M3Genum errorCode)
#endif
{
    M3G_VALIDATE_INTERFACE(m3g);
...
#   else /* !M3G_NGL_CONTEXT_API */
    ...
    ctx = eglCreateContext(g_display,
                           config,
                           NULL,
                           NULL);
    ...
    */
...
#else
    if (info!=0 && strstr((const char *)info, "HW")) {
        m3g->supportAntialiasing = M3G_TRUE;
    }
    else {
	...
	//printf("==============glinfo:%s==============",(const char *)info);
    if (info!=0 && strstr((const char *)info, "MBX")) {
...
#       endif
        ...
        m3g->func.releaseTarget = params->releaseTargetFunc;
...
#       if !defined(M3G_NGL_CONTEXT_API)
        ...
        if (eglQueryString(g_display, EGL_VERSION)) {
            ++m3g->glRefCount;
        }
#       endif /*!M3G_NGL_CONTEXT_API*/
