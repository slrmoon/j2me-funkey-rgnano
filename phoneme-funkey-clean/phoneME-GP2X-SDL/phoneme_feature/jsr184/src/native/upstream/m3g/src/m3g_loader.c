...
static M3Gbool m3gLoadNodeData(Loader *loader, M3GNode obj)
{
    ...
    {
        unsigned a = *data++;
        M3Gfloat alpha = (M3Gfloat) a / 255.0f;
        if (alpha < 0.0f) {
            alpha = 0.0f;
        }
        else if (alpha > 1.0f) {
...
static M3Gbool m3gLoadCompositingMode(Loader *loader)
{
    ...
    m3gSetBlending          (obj, *data++);
    {
        ...
        m3gSetAlphaThreshold(obj, alpha);
    }
...
M3G_API void m3gImportObjects(M3GLoader loader, M3Gint n, M3Gulong *refs)
{
    ...
    for (i = 0; i < n; ++i) {
        M3Gpointer ref = (M3Gpointer) refs[i];
        /* For loop is interrupted in case of OOM */
        /*
         * Imported external references are dependencies for the file being
         * decoded, not root objects of this loader. Store them as already
         * referenced so m3gGetLoadedObjects returns only objects introduced by
         * the current file.
         */
        ref |= 1;
