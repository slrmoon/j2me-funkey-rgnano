...
<xsl:template match="constant_class | localized_strings" mode="joinClasses">
    ...
    <xsl:if test="not(preceding::constant_class[concat(@Package,'.',@Name)=$className]
                  | preceding::localized_strings[concat(@Package,'.',@Name)=$className])">
        <!-- output matched node -->
        <xsl:copy>
            <!-- output all matched node attributes -->
            <xsl:copy-of select="@*"/>
            <!-- for each child of nodes with same class name -->
            <xsl:for-each select="ancestor::configuration/constants/constant_class[
                                      concat(@Package,'.',@Name)=$className]/child::*
                                  | ancestor::configuration/localized_strings[
                                      concat(@Package,'.',@Name)=$className]/child::*">
                <!-- output child node -->
...
<xsl:template match="node()[@KeysClass != '']" 
    ...
    <xsl:variable name="keysValuesNodes" 
        select="ancestor::configuration/constants/constant_class[$keysClassName=concat(
    ...
    <xsl:for-each select="$keysValuesNodes">
        ...
        <xsl:variable name="shortKeyName" select="@Name"/>
        
        <!-- nodes that use this key (refer to it) -->
        <xsl:variable name="keyUsersNodes" 
            select="ancestor::configuration/localized_strings[
                        concat(@Package,'.',@Name)=concat($nodePackage,'.',$nodeClass)
                        and @KeysClass=$keysClassName]/child::*[@Key=$shortKeyName]
                    | ancestor::configuration/skin/skin_properties[
                        @KeysClass=$keysClassName]/child::*[@Key=$shortKeyName]"/>
...
is used more than once in '<xsl:value-of select="$nodeName"/>'
    ...
    <xsl:copy>
        ...
        <xsl:for-each select="child::*">
            ...
            <xsl:variable name="shortKeyName" select="@Key"/>
        
            <!-- constant nodes providing value for this key -->
            <xsl:variable name="keyValueNodes" 
                select="ancestor::configuration/constants/
                    constant_class[@KeysValuesProvider='true'
                                   and $keysClassName=concat(@Package,'.',@Name)]
                    /constant[@Name=$shortKeyName]"/>
...
corresponding to key '<xsl:value-of select="@Key"/>'
            ...
            <xsl:copy>
                ...
                <xsl:attribute name="KeyValue">
                    <xsl:choose>
                        <xsl:when test="count($keyValueNodes)=0">
                            <xsl:value-of select="position()-1"/>
                        </xsl:when>
                        <xsl:otherwise>
