    ...
    for (b = y; b < y + height;
        ...
        for (a = x; a < x + width; a++) {
            ...
            if (a < clipX1) {
                ...
                if (sbuf->alphaData != NULL && processAlpha) {
                    sbuf->pixelData[idx] = GXJ_RGB24TORGB16(value);
                    sbuf->alphaData[idx] = (value >> 24) & 0xff;
                } else if (!processAlpha || ((value & 0xff000000) == 0xff000000)) {
                    sbuf->pixelData[idx] = GXJ_RGB24TORGB16(value);
                } else {
                    int alpha = (value >> 24) & 0xff;

                    if (alpha != 0) {
                        unsigned int dst = GXJ_RGB16TORGB24(sbuf->pixelData[idx]);
                        int invAlpha = 255 - alpha;
                        int srcR = (value >> 16) & 0xff;
                        int srcG = (value >> 8) & 0xff;
                        int srcB = value & 0xff;
                        int dstR = (dst >> 16) & 0xff;
                        int dstG = (dst >> 8) & 0xff;
                        int dstB = dst & 0xff;
                        int outR = (srcR * alpha + dstR * invAlpha + 127) / 255;
                        int outG = (srcG * alpha + dstG * invAlpha + 127) / 255;
                        int outB = (srcB * alpha + dstB * invAlpha + 127) / 255;
