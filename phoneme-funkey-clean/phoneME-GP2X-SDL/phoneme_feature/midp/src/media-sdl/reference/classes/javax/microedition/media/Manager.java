...
import javax.microedition.media.GenericPlayer;
...
public final class Manager {
    private static boolean audioSubsystemInitialized;
    static private native int nInitAudioSubsystem();
    static private native void nPlayTone(int None, int Duration, int Volume);
    /**
    ...
    static synchronized void ensureAudioSubsystem() throws MediaException {
        if (!audioSubsystemInitialized) {
            int initStatus = nInitAudioSubsystem();
            if (initStatus != 0) {
                throw new MediaException("Cannot initialize audio subsystem");
            }
            audioSubsystemInitialized = true;
        }
    }
    ...
    { Player Ret = null;
      ...
      try { if (locator.compareTo(TONE_DEVICE_LOCATOR) == 0) Ret = new ToneSequencePlayer();
                      ...
                      { String type = getContentTypeFromLocator(locator);
                        if (type == null) throw new MediaException("Cannot determine content type");
                        Ret = (ABBBasicPlayer)createPlayer(Connector.openInputStream(locator), type);
                      }
    ...
    { ABBBasicPlayer Ret;
      ...
      if (isMidiType(type))
         ...
         { try { Ret = new ToneSequencePlayer();
               } catch(Exception E) { throw new MediaException("Cannot create Player"); }
         }
    ...
    private static String getContentTypeFromLocator(String locator) {
      int q = locator.indexOf('?');
      if (q >= 0) locator = locator.substring(0, q);
      int dot = locator.lastIndexOf('.');
      if (dot < 0 || dot == locator.length() - 1) return null;
      String ext = locator.substring(dot + 1).toLowerCase();
      if (ext.equals("mid") || ext.equals("midi") || ext.equals("rmi") || ext.equals("kar")) return "audio/midi";
      if (ext.equals("wav") || ext.equals("wave")) return "audio/x-wav";
