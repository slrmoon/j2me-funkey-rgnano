...
static Instrument *create_synthetic_instrument(MidiSong *song, int percussion, int program)
{
  Instrument *ip;
  Sample *sp;
  sample_t *data;
  int wave_samples = 128;
  int i;
  int root_note;
  Sint32 loop_len;

  ip = safe_malloc(sizeof(Instrument));
  memset(ip, 0, sizeof(Instrument));
  ip->samples = 1;
  ip->sample = safe_malloc(sizeof(Sample));
  memset(ip->sample, 0, sizeof(Sample));

  sp = &ip->sample[0];
  data = safe_malloc(sizeof(sample_t) * wave_samples);
  for (i = 0; i < wave_samples; i++)
    {
      if (percussion)
        data[i] = (i & 1) ? 6000 : -6000;
      else
        data[i] = (i < (wave_samples / 2)) ? 12000 : -12000;
    }

  root_note = percussion ? program : 60;
  if (root_note < 0)
    root_note = 0;
  if (root_note > 127)
    root_note = 127;
  ...
  sp->data = data;
  sp->sample_rate = song->rate;
  sp->low_freq = freq_table[0];
  sp->high_freq = freq_table[127];
  sp->root_freq = freq_table[root_note];
  sp->low_vel = 0;
  sp->high_vel = 127;
  sp->volume = percussion ? 0.35f : 0.20f;
  sp->panning = 64;
  sp->note_to_use = percussion ? root_note : 0;
  sp->modes = MODES_16BIT | MODES_LOOPING | MODES_SUSTAIN;
  loop_len = wave_samples << FRACTION_BITS;
  sp->data_length = loop_len;
  sp->loop_start = 0;
  sp->loop_end = loop_len;
...
static int fill_bank(MidiSong *song, int dr, int b)
{
  ...
  for (i=0; i<128; i++)
      ...
      if (bank->instrument[i]==MAGIC_LOAD_INSTRUMENT)
	{
          ...
          if (Timidity_UsesSyntheticFallback() && song->patches == NULL)
            {
              bank->instrument[i] = create_synthetic_instrument(song, dr, i);
              continue;
            }
	  if (!(bank->tone[i].name))
	    {
