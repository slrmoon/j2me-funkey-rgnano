...
static Uint8 midi_laststatus;
static Uint8 midi_lastchan;
static Uint8 midi_nrpn;
static Uint8 midi_rpn_msb[16], midi_rpn_lsb[16];
  ...
  for (;;)
      ...
      else if(me==0xFF) /* Meta event */
	{
	  ...
	  if (type>0 && type<16)
		...
		if (len > 3)
		  SDL_RWseek(song->rw, len - 3, SEEK_CUR);
		break;
      ...
      else
	{
	  ...
	  switch(midi_laststatus)
	    ...
	    case 0: /* Note off */
	      SDL_RWread(song->rw, &b, 1, 1);
	      b &= 0x7F;
	      MIDIEVENT(song->at, ME_NOTEOFF, midi_lastchan, a,b);

	    case 1: /* Note on */
	      SDL_RWread(song->rw, &b, 1, 1);
	      b &= 0x7F;
	      MIDIEVENT(song->at, ME_NOTEON, midi_lastchan, a,b);

	    case 2: /* Key Pressure */
	      SDL_RWread(song->rw, &b, 1, 1);
	      b &= 0x7F;
	      MIDIEVENT(song->at, ME_KEYPRESSURE, midi_lastchan, a, b);
		...
		switch(a)
		  ...
		  case 100: midi_nrpn=0; midi_rpn_msb[midi_lastchan]=b; break;
		  case 101: midi_nrpn=0; midi_rpn_lsb[midi_lastchan]=b; break;
		  case 99: midi_nrpn=1; midi_rpn_msb[midi_lastchan]=b; break;
		  case 98: midi_nrpn=1; midi_rpn_lsb[midi_lastchan]=b; break;
		    
		  case 6:
		    if (midi_nrpn)
		      {
			SNDDBG(("(Data entry (MSB) for NRPN %02x,%02x: %d)\n",
				midi_rpn_msb[midi_lastchan], midi_rpn_lsb[midi_lastchan], b));
			break;
		      }
			...
			MIDIEVENT(song->at, ME_PITCH_SENS, midi_lastchan, 2, 0);
		...
		if (control != 255)
		  { 
		    MIDIEVENT(song->at, control, midi_lastchan, b, 0); 
		  }
	      }
	      break;
	    ...
	    case 4: /* Program change */
	      a &= 0x7f;
	      MIDIEVENT(song->at, ME_PROGRAM, midi_lastchan, a, 0);
	    ...
	    case 6: /* Pitch wheel */
	      SDL_RWread(song->rw, &b, 1, 1);
	      b &= 0x7F;
	      MIDIEVENT(song->at, ME_PITCHWHEEL, midi_lastchan, a, b);

	    default: 
	      SNDDBG(("*** Can't happen: status 0x%02X, channel 0x%02X\n",
		      midi_laststatus, midi_lastchan));
	      break;
	    }
	}
    }
