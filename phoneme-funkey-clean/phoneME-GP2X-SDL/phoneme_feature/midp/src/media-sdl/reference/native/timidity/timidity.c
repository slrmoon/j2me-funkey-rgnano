...
static int synthetic_fallback_enabled = 0;
...
static int add_path_if_present(const char *path)
{
    if (path == NULL || *path == '\0')
        return 0;

    add_to_pathlist((char *)path);
    return 1;
}

static void add_midp_bank_paths(void)
{
    char path[1024];
    const char *midp_home = midpGetHomeDir();

    if (midp_home == NULL || *midp_home == '\0')
        return;

    snprintf(path, sizeof(path), "%s/lib", midp_home);
    add_path_if_present(path);
    snprintf(path, sizeof(path), "%s/lib/freepats", midp_home);
    add_path_if_present(path);
}
...
static int try_config_file(const char *name)
{
    if (name == NULL || *name == '\0')
        return -1;

    return read_config_file((char *)name);
}

int Timidity_Init_NoConfig()
{
  synthetic_fallback_enabled = 1;
...
int Timidity_Init()
{
  ...
  env_dir = getenv("PHONEME_TIMIDITY_DIR");
  add_path_if_present(env_dir);
  add_midp_bank_paths();
  Timidity_Init_NoConfig();
  env_synthetic = getenv("PHONEME_TIMIDITY_SYNTHETIC");
  if (env_synthetic != NULL && strcmp(env_synthetic, "0") != 0)
    {
      synthetic_fallback_enabled = 1;
      return 0;
    }
  env_cfg = getenv("PHONEME_TIMIDITY_CFG");
  status = try_config_file(env_cfg);
  if (status < 0)
    status = try_config_file(CONFIG_FILE);
  if (status < 0)
    status = try_config_file("freepats.cfg");
  if (status < 0)
    {
      synthetic_fallback_enabled = 1;
      return 0;
    }
  synthetic_fallback_enabled = 0;
  return status;
}

int Timidity_UsesSyntheticFallback(void)
{
