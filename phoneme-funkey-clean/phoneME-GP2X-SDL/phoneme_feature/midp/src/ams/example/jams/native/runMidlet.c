...
static int getMidpHeapRequirement() {
    char *heapKb = getenv("PHONEME_HEAP_KB");
    char *heapMb = getenv("PHONEME_HEAP_MB");
    int value;

    if (heapKb != NULL && heapKb[0] != '\0') {
        value = atoi(heapKb);
...
runMidlet(int argc, char** commandlineArgs) {
    ...
    fprintf(stderr, "runMidlet: argc=%d\n", argc);
    for (i = 0; i < argc; i++) {
        fprintf(stderr, "runMidlet: argv[%d]=%s\n", i,
                commandlineArgs[i] == NULL ? "(null)" : commandlineArgs[i]);
    }
     ...
     */
    {
        int heapRequirement = getMidpHeapRequirement();
        fprintf(stderr, "runMidlet: heap=%d bytes\n", heapRequirement);
        JVM_SetConfig(JVM_CONFIG_HEAP_CAPACITY, heapRequirement);
    }
    ...
    do {
        ...
        do {
            const jbyte* classNameChars = NULL;

            if (!pcsl_string_is_null(&classname)) {
                classNameChars = pcsl_string_get_utf8_data(&classname);
            }

            fprintf(stderr,
                    "runMidlet: calling suiteId=%ld class=%s additionalPath=%s debug=%d repeat=%d\n",
                    (long)suiteId,
                    classNameChars == NULL ? "(null)" : (const char*)classNameChars,
                    additionalPath == NULL ? "(null)" : additionalPath,
                    debugOption,
                    repeatMidlet);

            if (classNameChars != NULL) {
                pcsl_string_release_utf8_data(classNameChars, &classname);
            }
            ...
            fprintf(stderr, "runMidlet: returned status=%d\n", status);
