...
midpInitializeUI(void) {
    fprintf(stderr, "midpInitializeUI: begin\n");

    if (InitializeEvents() != 0) {
        fprintf(stderr, "midpInitializeUI: InitializeEvents failed\n");
        return -1;
    }
    fprintf(stderr, "midpInitializeUI: InitializeEvents ok\n");
    ...
    if (pushopen() != 0) {
        fprintf(stderr, "midpInitializeUI: pushopen failed\n");
        return -1;
    }
    fprintf(stderr, "midpInitializeUI: pushopen ok\n");

    {
        int uiStatus = lcdlf_ui_init();
        fprintf(stderr, "midpInitializeUI: lcdlf_ui_init=%d\n", uiStatus);
        fflush(stderr);
...
midp_run_midlet_with_args_cp(SuiteIdType suiteId,
    ...
    fprintf(stderr,
            "midp_run: enter suite=%ld ext=%s\n",
            (long)suiteId,
            classPathExt == NULL ? "(null)" : classPathExt);

    {
        int initStatus = midpInitCallback(VM_LEVEL, midpInitializeUI,
                                          midpFinalizeUI);
        fprintf(stderr, "midp_run: midpInitCallback(VM_LEVEL)=%d\n",
                initStatus);
        if (initStatus != 0) {
    ...
    if (init_jsr120() != WMA_NET_SUCCESS) {
        ...
        fprintf(stderr, "midp_run: init_jsr120 failed\n");
        return MIDP_ERROR_STATUS;
    }
#endif

    commandState = midpGetCommandState();
    fprintf(stderr, "midp_run: commandState=%p\n", commandState);
    ...
    if (! pcsl_string_is_null(midletClassName)) {
        ...
        if (PCSL_STRING_OK != res) {
            ...
            fprintf(stderr, "midp_run: dup classname failed res=%d\n", res);
            return MIDP_ERROR_STATUS;
        }
    }
    ...
    if (! pcsl_string_is_null(arg0)) {
        ...
        if (PCSL_STRING_OK != res) {
            ...
            fprintf(stderr, "midp_run: dup arg0 failed res=%d\n", res);
            return MIDP_ERROR_STATUS;
        }
    }
    ...
    if (! pcsl_string_is_null(arg1)) {
        ...
        if (PCSL_STRING_OK != res) {
            ...
            fprintf(stderr, "midp_run: dup arg1 failed res=%d\n", res);
            return MIDP_ERROR_STATUS;
        }
    }
    ...
    if (! pcsl_string_is_null(arg2)) {
        ...
        if (PCSL_STRING_OK != res) {
            ...
            fprintf(stderr, "midp_run: dup arg2 failed res=%d\n", res);
            return MIDP_ERROR_STATUS;
        }
    }
    ...
    do {
        ...
        case MIDP_ERROR_OUT_MEM:
            ...
            fprintf(stderr, "midp_run: getClassPathPlus OUT_MEM suite=%ld ext=%s\n",
                    (long)commandState->suiteId,
                    classPathExt == NULL ? "(null)" : classPathExt);
            return MIDP_ERROR_STATUS;

        case MIDP_ERROR_AMS_SUITE_CORRUPTED:
            REPORT_WARN(LC_CORE, "I/O error reading appdb.\n");
            fprintf(stderr, "midp_run: getClassPathPlus SUITE_CORRUPTED suite=%ld ext=%s\n",
                    (long)commandState->suiteId,
                    classPathExt == NULL ? "(null)" : classPathExt);
            return MIDP_ERROR_STATUS;
        ...
        case MIDP_ERROR_AMS_SUITE_NOT_FOUND:
            ...
            fprintf(stderr, "midp_run: getClassPathPlus SUITE_NOT_FOUND suite=%ld ext=%s\n",
                    (long)commandState->suiteId,
                    classPathExt == NULL ? "(null)" : classPathExt);
         ...
         */
        fprintf(stderr, "midp_run: starting VM suite=%ld ext=%s\n",
                (long)commandState->suiteId,
                classPathExt == NULL ? "(null)" : classPathExt);
#if !USE_UNICODE_FOR_FILENAMES
        fprintf(stderr, "midp_run: classpath=%s\n",
                classPath == NULL ? "(null)" : (char*)classPath);
#endif
        vmStatus = midpRunVm(classPath, MIDP_MAIN, 0, NULL);
        fprintf(stderr, "midp_run: midpRunVm returned %d commandStatus=%d lastSuite=%ld\n",
                vmStatus, commandState->status, (long)commandState->lastSuiteId);
